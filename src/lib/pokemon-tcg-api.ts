...
import {
  ...
  type OfficialJapaneseBrowseItem,
...
const TCGDEX_COLLECTOR_TIMEOUT_MS = 800;
...
async function findPartialCollectorInQuery(trimmed: string) {
  ...
  for (const match of trimmed.matchAll(tokenPattern)) {
    ...
    if (!/^\d{1,4}$/.test(match[1]) && (await isLikelySetCodeToken(match[1]))) {
      continue;
    }
...
async function extractSetContextFromQuery(
  ...
  if (
    isSimpleNameSearchQuery(trimmed) ||
    isCollectorNumberSearchQuery(trimmed) ||
    isNamePlusPartialCollectorQuery(trimmed)
  ) {
...
// order). Price sorts use the API's trendPrice order for page 1 so Dex stays
// inside the 15s budget; the client re-sorts as live prices hydrate.
...
function inferTcgdexBriefCategory(brief: TcgdexCardBrief) {
  if (typeof brief.category === "string" && brief.category.trim()) {
    return brief.category;
  }

  if (/\benergy\b/i.test(brief.name)) {
    return "Energy";
  }

  if (
    /\b(trainer|supporter|stadium|tool|item|candy|switch|research|communication|ball)\b/i.test(
      brief.name,
    )
  ) {
    return "Trainer";
...
function tcgdexBriefToCardResponse(brief: TcgdexCardBrief): TcgdexCardResponse {
  const setId = getSetIdFromTcgdexCardId(brief.id, brief.localId) || brief.id;
  ...
  return {
    ...
    name: brief.name,
    image: brief.image,
    category: inferTcgdexBriefCategory(brief),
    set: {
      id: setId,
      name: setId,
    },
...
function identityCardsFromTcgdexBriefs(
  briefs: TcgdexCardBrief[],
  language: CardLanguageCode,
  englishName?: string,
): TcgCard[] {
  return briefs.map((brief) =>
    normalizeTcgdexCard(tcgdexBriefToCardResponse(brief), language, {
      name:
        language === "ja"
          ? englishName || inferEnglishNameFromTcgdexLocalizedName(brief.name)
          : brief.name,
    }),
  );
}

function identityCardsFromOfficialJapaneseSeed({
  aliases,
  englishName,
  page,
  pageSize,
  ...
  const seed = searchOfficialJapaneseBrowseSeed({ aliases, page, pageSize });

  return {
    cards: seed.matches.map((match) =>
      normalizeOfficialJapaneseCard(
        buildOfficialJapaneseDetailFromBrowseItem(
          match.item,
          match.setIndex,
...
function collectorIntentFromQuery(query: string): {
  collectorCode: CollectorCodeQuery;
  nameQuery: string;
} | null {
  const trimmed = query.trim();
  ...
  if (!isNamePlusPartialCollectorQuery(trimmed)) {
    return null;
  }

  const parts = trimmed.split(/\s+/).filter(Boolean);
  let collectorCode: CollectorCodeQuery | null = null;
  const nameParts: string[] = [];

  for (const part of parts) {
    const parsed = parsePartialCollectorToken(part);
    if (parsed && !collectorCode) {
      collectorCode = parsed;
    } else {
      nameParts.push(part);
    }
  }

  return collectorCode ? { collectorCode, nameQuery: nameParts.join(" ") } : null;
}

function pageInstantIdentityResults(
  results: SearchResult[],
  page: number,
  pageSize: number,
  sort: SearchSortOption,
  ...
  const sorted = applySearchResultSort(results, sort);
...
/**
 * Local-only Dex identities: official Japanese browse seed, collector fallbacks,
 * and the bundled card index. Used so set/name/number searches never wait on
 * pokemon-card.com or TCGdex before first paint.
 */
export async function instantIdentitySearchResponse(
  query: string,
  setFilter: string | undefined,
  page: number,
  language: CardLanguageFilter,
  ...
  const pageSize =
    resolvedLanguage === "all" ? SEARCH_PAGE_SIZE : LOCALIZED_SEARCH_PAGE_SIZE;
  const cleanQuery = query.trim();
  const collectorIntent = collectorIntentFromQuery(cleanQuery);

  if (resolvedSetFilter && (resolvedLanguage === "ja" || resolvedLanguage === "all")) {
    const seededSet = searchJapaneseSetFromSeed(
      resolvedSetFilter,
      page,
      pageSize,
      sort,
      collectorIntent ? "" : cleanQuery,
    );
    if (seededSet?.results.length) {
      return seededSet;
    }
  }

  if (collectorIntent && (resolvedLanguage === "ja" || resolvedLanguage === "all")) {
    const cards = officialJapaneseFallbackCollectorCards(
      collectorIntent.collectorCode,
      collectorIntent.nameQuery,
    );
    if (cards.length) {
      return pageInstantIdentityResults(
        cards.map((card) => ({
          card,
          score: 150,
          matchReason: `Official Japanese collector ${collectorCodeDisplayLabel(collectorIntent.collectorCode)}`,
        })),
        page,
        pageSize,
        sort,
  ...
  if (
    ...
    const aliases = await findDbLocalizedPokemonNameAliases(cleanQuery, "ja").catch(
      () => [] as string[],
    );
    const official = identityCardsFromOfficialJapaneseSeed({
      aliases: aliases.length ? aliases : [cleanQuery],
      englishName: cleanQuery,
      page,
      pageSize,
    });
    const officialResults: SearchResult[] = official.cards.map((card) => ({
      card,
      score: 138,
    ...
    const bundled =
      resolvedLanguage === "all"
        ? bundledCatalogFallbackResponse(
            cleanQuery,
            undefined,
            page,
            "all",
    ...
    if (merged.length) {
      return pageInstantIdentityResults(
        ...
        pageSize,
        sort,
        official.totalCount ?? merged.length,
      );
    }
  }

  return bundledCatalogFallbackResponse(
    cleanQuery,
    resolvedSetFilter,
    page,
    resolvedLanguage,
...
function officialJapaneseFallbackCollectorCards(
  collectorCode: CollectorCodeQuery,
  nameQuery = "",
): TcgCard[] {
  const fullMatch = isFullCollectorCode(collectorCode)
    ? lookupOfficialJpCollectorFallback(collectorCode)
    : null;
  const partialMatch = fullMatch
    ? null
    : lookupOfficialJapanesePartialCollectorFallback(collectorCode, nameQuery);
  const resolvedCode = fullMatch
    ? collectorCode
    : partialMatch?.fullCode;
  const fallback = fullMatch ?? partialMatch?.fallback;

  if (!resolvedCode || !fallback || !isFullCollectorCode(resolvedCode)) {
    return [];
  }

  const card = normalizeOfficialJapaneseCard(
    buildOfficialJapaneseFallbackDetail(resolvedCode, fallback),
    fallback.englishName,
  );

  if (!collectorCardMatchesNameHint(card, nameQuery)) {
    return [];
  }
...
function searchJapaneseSetFromSeed(
  ...
  let setCode = setFilter;

  for (const code of codes) {
    const candidate = getOfficialJapaneseBrowseSeedAllItems(code);
  ...
  const supplement = getOfficialJapaneseSetSupplementById(setCode);
  const printedTotal = supplement?.printedTotal ?? supplement?.total ?? seeded.hitCnt;
  let cards = seeded.items.map((item, index) =>
    normalizeOfficialJapaneseCard(
      buildOfficialJapaneseDetailFromBrowseItem(item, index, setCode, printedTotal),
    ),
  );
  ...
  if (cleanQuery.trim()) {
    cards = cards.filter((card) => {
      const searchable = [
        ...
        .filter(Boolean)
        .join(" ");
      return textMatchesQuery(searchable, cleanQuery);
    });
  }
  ...
  return makeSearchResponse({
    ...
    page,
    pageSize,
    hasNextPage: startIndex + pageSize < results.length,
  });
}
...
async function resolveLocalizedSetIdCandidates(
  ...
  if (
    language === "ja" &&
    (hasOfficialJapaneseBrowseSeedSet(setFilter) || isLikelyOfficialJapaneseSetCode(setFilter))
  ) {
...
async function searchPartialCollectorAllLanguages(
  ...
  const staticJa = officialJapaneseFallbackCollectorCards(collectorCode, nameQuery);
  const emptyEnglish: LiveSearchResponse = {
    results: [],
    totalCount: 0,
    page: normalizedPage,
    pageSize: SEARCH_PAGE_SIZE,
  ...
  const live = await withSearchBudget(
    ...
    staticJa.length ? 500 : 1_600,
    null,
  );

  const [englishResponse, ...localizedResponses] = live ?? [emptyEnglish];
  ...
  const merged = [
    ...staticJa.map((card) => ({
      card,
      score: 170,
      matchReason: `Official Japanese collector ${partialLabel}${nameQuery ? ` with ${nameQuery}` : ""}`,
    })),
...
async function fetchLocalizedCardsByCollectorCode(
  ...
  return briefsToDetail
    .map((brief) => tcgdexBriefToCardResponse(brief))
    .filter((card) => {
      ...
      if (setKeys.length) {
        ...
        if (!setKeys.some((key) => setId === key || setId.includes(key))) {
          return false;
        }
      }
      ...
      if (typeof official !== "number" && typeof setTotal !== "number") {
        return false;
      }

      return official === collectorCode.printedTotal || setTotal === collectorCode.printedTotal;
    });
}
...
async function searchLocalizedCollectorCodeResults(
  ...
  const staticOfficial =
    language === "ja"
      ? officialJapaneseFallbackCollectorCards(collectorCode, nameQuery)
      : [];
  const marketFallback =
    !staticOfficial.length && (language === "zh-cn" || language === "zh-tw")
      ? lookupCollectorMarketFallback(collectorCode, language, nameQuery)
      : null;
  const officialJapaneseCards = marketFallback
    ? [
        buildCollectorCodeMarketFallbackBaseCard(
    ...
    : staticOfficial;
  const tcgdexMatches = await withSearchBudget(
    fetchLocalizedCardsByCollectorCode(collectorCode, language, {
      nameQuery,
      localizedNameAliases: options.localizedNameAliases,
    }),
    staticOfficial.length || officialJapaneseCards.length ? 400 : 800,
    [] as TcgdexCardResponse[],
  );

  const normalizedTcgdexCards = tcgdexMatches.length
    ? identityCardsFromTcgdexBriefs(
        tcgdexMatches.map((card) => ({
          id: card.id,
  ...
  return makeSearchResponse({
    ...
    notice:
      officialJapaneseCards.length && normalizedTcgdexCards.length
        ...
        : officialJapaneseCards.length
          ? `Matched collector code ${exactCode} from the official Japanese identity catalog.`
          : undefined,
  });
}
...
async function searchCollectorCodeAllLanguages(
  ...
  const exactCode = collectorCodeLabel(collectorCode);
  const staticOfficialCards = officialJapaneseFallbackCollectorCards(collectorCode, nameQuery);
  const staticOfficialResults: SearchResult[] = staticOfficialCards.map((card) => ({
    card,
    score: collectorCodeSearchScore(card, "ja"),
    matchReason: `Official Japanese collector code ${exactCode}`,
  }));
  const emptyEnglishPayload: PokemonTcgCardApiResponse = {
    data: [],
    totalCount: 0,
    page: 1,
    pageSize: SEARCH_PAGE_SIZE,
  };
  const emptyLocalized: LiveSearchResponse = {
    results: [],
    totalCount: 0,
    page: 1,
    pageSize: SEARCH_PAGE_SIZE,
  ...
  const [englishPayload, jaLocalized, indexResults] = await Promise.all([
    withSearchBudget(
      fetchCardSearchPage(
        [
          buildFullCollectorCodeLuceneClause(collectorCode),
        ...
        SEARCH_PAGE_SIZE,
        "-set.releaseDate,number",
      ).catch(() => emptyEnglishPayload),
      staticOfficialResults.length ? 500 : 900,
      emptyEnglishPayload,
    ),
    withSearchBudget(
      searchLocalizedCollectorCodeResults(collectorCode, "ja", {
        nameQuery,
        localizedNameAliases,
        page: 1,
        pageSize: SEARCH_PAGE_SIZE,
        sort: "relevance",
      }).catch(() => emptyLocalized),
      staticOfficialResults.length ? 500 : 900,
      emptyLocalized,
    ),
    withSearchBudget(
      buildIndexCollectorSearchResults(collectorCode, "all", nameQuery, localizedNameAliases),
      250,
      [] as SearchResult[],
    ),
  ]);
  ...
  const localizedResults = jaLocalized.results;
  let merged = [...staticOfficialResults, ...localizedResults, ...englishResults, ...indexResults];
  ...
  if (!deduped.length) {
    const cachedCards = (
      await withSearchBudget(
        lookupCachedCardsByCollectorCode("all", collectorCode),
        200,
        [] as TcgCard[],
      )
    ).filter((card) => collectorCardMatchesNameHint(card, nameQuery, localizedNameAliases));
    ...
    const heuristic = await withSearchBudget(
      searchCollectorHeuristicEnglish(collectorCode, normalizedPage),
      400,
      null,
    );
    if (heuristic) {
...
async function searchLocalizedCardsByEnglishQuery(
  ...
  const officialJapanese =
    language === "ja" && includeOfficialJapanese
      ? identityCardsFromOfficialJapaneseSeed({
          aliases: localizedNameAliases.length ? localizedNameAliases : localizedNameQueries,
          englishName: cleanQuery,
          page: normalizedPage,
          pageSize: itemsPerPage,
        })
  ...
  let uniqueLocalizedBriefs = dedupeTcgdexBriefs(localizedAliasBriefs).slice(
    0,
    LOCALIZED_ALIAS_BRIEF_LIMIT,
  );

  if (
    !uniqueLocalizedBriefs.length &&
    !officialJapanese.cards.length &&
    isLikelyEnglishCatalogQuery(cleanQuery)
  ) {
    const englishBriefs = await fetchTcgdexJson<TcgdexCardBrief[]>(
      `${TCGDEX_API_BASE_URL}/en/cards?${new URLSearchParams({
        "pagination:page": "1",
        "pagination:itemsPerPage": String(Math.min(8, itemsPerPage)),
        name: cleanQuery,
      }).toString()}`,
      { timeoutMs: Math.min(500, catalogTimeoutMs) },
    ).catch(() => [] as TcgdexCardBrief[]);
    const localizedFromEnglish = (
      await Promise.all(
        englishBriefs.slice(0, 8).map((brief) =>
          fetchTcgdexJson<TcgdexCardResponse>(
            `${TCGDEX_API_BASE_URL}/${apiLanguage}/cards/${encodeURIComponent(brief.id)}`,
            { timeoutMs: 350 },
          ).catch(() => null),
        ),
      )
    ).filter((card): card is TcgdexCardResponse => Boolean(card?.id));

    uniqueLocalizedBriefs = localizedFromEnglish.map((card) => ({
      id: card.id,
      localId: card.localId,
      name: card.name,
      image: card.image,
  ...
  if (!uniqueLocalizedBriefs.length && !officialJapanese.cards.length) {
    return makeSearchResponse({
      results: [],
      totalCount: 0,
      page: normalizedPage,
  ...
  const aliasCards = identityCardsFromTcgdexBriefs(
    uniqueLocalizedBriefs.slice(0, itemsPerPage + 4),
    language,
    cleanQuery,
  );
  const uniqueCards = aliasCards.filter(
    (card, index, cards) => cards.findIndex((item) => item.id === card.id) === index,
  );
  const normalizedCards = uniqueCards.slice(0, itemsPerPage);
  const hydratedCards = normalizedCards;
  ...
  const pricedResults = options.skipPriceHydrate
    ? applyEarlyMarketSearchEstimates(mergedWithEnglishNames)
    : applyEarlyMarketSearchEstimates(
  ...
  return makeSearchResponse({
    ...
    hasNextPage:
      (typeof officialJapanese.totalCount === "number"
        ...
        : false) || uniqueLocalizedBriefs.length >= itemsPerPage,
    notice: results.length
...
export async function fetchSearchSets(
  ...
  const bundled = trimmedQuery
    ? ((await searchSetsInDatabase(trimmedQuery, language)) ?? [])
    : getBundledSetsCatalog(language);
  const local = bundled.length
    ? bundled
    : trimmedQuery
      ? await searchSetsInDatabase(trimmedQuery, language)
      : await withSearchBudget(getSetsFromDatabase(language), 400, bundled);
  const livePromise = rememberLiveSearchSets(language);
  const liveWaitMs = local?.length ? 250 : 800;
...
async function searchLocalizedCards(
  ...
  if (cleanQuery && isLikelyEnglishCatalogQuery(cleanQuery)) {
    return searchLocalizedCardsByEnglishQuery(
      cleanQuery,
      normalizedPage,
      language,
      itemsPerPage,
      true,
      sort,
      { deadlineMs: 1_600, skipPriceHydrate: true },
    );
  }
  ...
  if (cleanQuery && !isLikelyEnglishCatalogQuery(cleanQuery)) {
    ...
    for (const englishTerm of englishTerms.slice(0, 3)) {
      const englishNameMatches = await searchLocalizedCardsByEnglishQuery(
        ...
        true,
        sort,
        { deadlineMs: 1_200, skipPriceHydrate: true },
      );
  ...
  const normalizedCards = identityCardsFromTcgdexBriefs(
    uniqueBriefs.slice(0, browseLimit),
    language,
    isLikelyEnglishCatalogQuery(cleanQuery) ? cleanQuery : undefined,
  );
  const pricedCards = normalizedCards;
...
function unwrapIncomingSetFilter(
  setFilter: string | undefined,
  language: CardLanguageFilter,
): { setFilter?: string; language: CardLanguageFilter } {
  const decoded = decodeSetFilterValue(setFilter);
  return {
    setFilter: decoded.setFilter || undefined,
    language: language === "all" && decoded.languageHint ? decoded.languageHint : language,
  };
}
...
async function searchLiveCardsUncached(
  ...
  if (resolvedLanguage === "all") {
    return searchAllLanguageCards(query, resolvedSetFilter, page, sort);
  }

  let localizedQuery = query.trim();
  let effectiveSetFilter = resolvedSetFilter;
  const normalizedPage = Number.isFinite(page) && page > 0 ? Math.floor(page) : 1;

  if (!effectiveSetFilter && localizedQuery) {
    const setContext = await extractSetContextFromQuery(localizedQuery, resolvedLanguage);
  ...
  if (isTrainerGallerySetFilter(effectiveSetFilter)) {
    return searchTrainerGalleryNameQuery(localizedQuery, normalizedPage, resolvedLanguage, sort);
  }
  ...
  if (isPriceAwareSort(sort)) {
    const pricedCount = results.filter((result) => result.card.marketPriceUsd > 0).length;
    if (pricedCount < Math.min(8, results.length)) {
...
function searchShouldHydrateJapanesePrices(
  language: CardLanguageFilter,
  sort: SearchSortOption = DEFAULT_SEARCH_SORT,
) {
  return language === "ja" && isPriceAwareSort(sort);
}
...
export async function searchLiveCards(
  ...
  const resolvedLanguage = incoming.language;
  const startedAt = Date.now();
  const cacheKey = `${makeSearchResultCacheKey(query, resolvedSetFilter, page, resolvedLanguage, sort)}|editions`;
  ...
  const unfinalized = await searchLiveCardsUnfinalized(
    query,
    resolvedSetFilter,
    page,
    resolvedLanguage,
  ...
  const isSetScoped = Boolean(resolvedSetFilter?.trim());
  ...
  // Name and card-number sorts paint identities immediately. Tile prices fill
  // from /api/price. Only price-aware set browses hydrate guides before paging.
  if (!isNameQuery && isPriceAwareSort(sort) && remainingMs > 250) {
    const pricedShare =
...
async function searchLiveCardsUnfinalized(
  ...
  if (cached) {
    const repairedCached = searchShouldHydrateJapanesePrices(language, sort)
      ? await hydrateJapaneseSearchResponse(cached, { maxHeadFetches: 0 })
      : cached;
    if (searchShouldHydrateJapanesePrices(language, sort)) {
  ...
  const searchPromise = (async () => {
    ...
    const localFallbackPromise = buildLocalCatalogFallbackResponse(
      query,
      setFilter,
      normalizedPage,
      language,
    ...
    try {
      ...
      let response = await withSearchTimeout(
        (async () => {
          ...
          if (language === "ja" && remainingMs > 200 && (isSetBrowse || isPriceAwareSort(sort))) {
      ...
      if (
        ...
        if (!instant?.results.length) {
          instant = await instantIdentitySearchResponse(
            query,
            setFilter,
            normalizedPage,
            language,
...
export async function fetchLiveCardBySlug(
  ...
  if (language !== "en") {
    ...
    if (language === "ja" && id.startsWith("official-")) {
      ...
      if (seedOrFallbackCard) {
        return withSlugFinish(seedOrFallbackCard);
      }

      const detail = await fetchOfficialJapaneseCardDetail(cardId, undefined, {
        timeoutMs: 800,
      }).catch(() => null);
...
async function searchEnglishNameAllLanguages(
  ...
  const results = applySearchResultSort(applyEarlyMarketSearchEstimates(merged), sort);
