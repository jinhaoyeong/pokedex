...
async function fetchSetCards(setId) {
  ...
  const cards = (payload.results ?? [])
    ...
    .filter((card) => card && (card.marketPriceUsd ?? 0) >= MIN_PRICE_USD);
  ...
  return [...new Map(picks.map((card) => [card.id, card])).values()].slice(0, SAMPLES_PER_SET);
}
...
async function validateCard(setMeta, card) {
  const gradingParams = buildGradingParams(card);
  ...
  const [gradingPayload, tcgReference] = await Promise.all([
    ...
    fetchTcgdexReference(setMeta.set_id, card.collectorNumber, card.id),
  ]);
