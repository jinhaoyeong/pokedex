...
**Market data:** first-paint Grade Values come from the first-party guide (`data/pokedex-market-guide.json` seed ∪ aggregated binder/vault observations in Postgres `market_observations`) plus free catalog identity/prices (Pokemon TCG API / TCGdex). When those have no PSA 9/10 book, `slab-estimate-v2` shows labelled **Estimate** rows from exact-print catalog raw × era/rarity, compresses runaway premiums on high-value raw cards, and learns guarded cohort ratios from authenticated first-party observations. Estimates never write to `market_observations`, seed export, headline consensus, or binder/portfolio totals. Rollback with `ESTIMATED_GRADE_VALUES=false`. `FIRST_PARTY_MARKET_ONLY` defaults on: grading and population do not call competitor APIs or scrape HTML; set it to `false` only to restore legacy external enrichment and optional eBay Browse validation. When no verified grading-company population snapshot exists, the UI may show a separately labelled PokePokedex collection census aggregated anonymously from cloud-vault graded holdings; it is never presented as official cards graded. PriceCharting, Collectr, and similar appraisal APIs are not required. Identity failures and widened price conflicts are stored in `market_estimate_diagnostics` when Postgres is configured, otherwise structured server logs.
